...
after_login_dir = "{}/after_login.py".format(os.getcwd())
plat = Plat_define()
platform = plat.use_platform()
if platform == 'mac':
	#appscript.app('Terminal').do_script(cmd)
    p = subprocess.Popen(['usr/bin/osascript'], shell = False, stdin = subprocess.PIPE, stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
    p.stdin.write('ls')
    #os.system("open -W -a Terminal.app")
